...
Page({
  ...
  data: {
    ...
    upParamsEnglishName: ['serverIP', 'port', 'gate', 'productKey', 'deviceSecret', 'deviceName', 'publishTopic', 'subscribeTopic', 'readCycle'],
    upParamsChineseName: ['服务器：', '端口：', '网关ID：', 'ProdKey：', 'DevSecret：', 'DevName：', '发布Topic：', '订阅Topic：', '抄表周期：'],
          ...
          if (params.operation == 0) {
            ...
            if (resultJSON.status == 1 && that.data.downParams.length < that.data.deviceCount) {
              ...
              for (var i = 0; i < that.data.downParams.length; i++) {
                console.log(resultJSON.device[0].block)
